...
export default function SpotTheDifferenceGame() {
  ...
  useEffect(() => {
    if (foundDifferences.size === gameConfig.differences.length && gameStarted) {
 ...
 console.log("gameConfig", gameConfig.images.image1);
  const handleImageClick = (event, imageIndex) => {
    ...
    const clickedDifference = gameConfig.differences.findIndex((diff, index) => {
      if (foundDifferences.has(index)) return false
      return scaledX >= diff.x && scaledX <= diff.x + diff.width && scaledY >= diff.y && scaledY <= diff.y + diff.height
    })
  ...
  const renderDifferenceMarkers = (imageIndex) => {
    return gameConfig.differences.map((diff, index) => {
      if (!foundDifferences.has(index)) return null
      return (
        <div
          key={index}
      ...
      <div className="max-w-7xl mx-auto">
        <Card className="mb-6">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold text-gray-800">{gameConfig.gameTitle}</CardTitle>
            <p className="text-gray-600">
              Find all {gameConfig.differences.length} differences between the two images!
            </p>
            <HowToPlay />
          </CardHeader>
        </Card>
          ...
          <Card className="mb-6 bg-green-50 border-green-200">
            <CardContent className="text-center p-6">
              ...
              <p className="text-green-700">
                You found all {gameConfig.differences.length} differences in {formatTime(elapsedTime)}!
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="overflow-hidden">
            ...
            <CardContent className="p-0">
              <div ref={imageRef} className="relative cursor-crosshair" onClick={(e) => handleImageClick(e, 0)}>
                <img
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent className="p-0">
              <div className="relative cursor-crosshair" onClick={(e) => handleImageClick(e, 1)}>
                <img
